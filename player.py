    ...
    def input(self):
        if not self.attacking:  # make it so player can't attack while moving in another direction, player can't do anything during an attack
            ...
            if keys[pygame.K_c]:
                self.attacking = True
                self.attack_time = pygame.time.get_ticks()  # called once and only once only
                style = list(magic_data.keys())[
                    self.magic_index]  # make a list of all the styles, flame or heal
                strength = list(magic_data.values())[
                    self.magic_index]['strength'] + self.stats['magic']  # make a list of only strength + the base stats of player magic power
                cost = list(magic_data.values())[
                    self.magic_index]['cost']
                self.create_magic(style, strength, cost)

            # switching weapons input
            ...
            # switching magic input
            if keys[pygame.K_e] and self.can_switch_magic:
                self.can_switch_magic = False
                self.magic_switch_time = pygame.time.get_ticks()  # called once and only once only
