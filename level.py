...
class Level:
    def __init__(self):
        ...
        # check collisions between these two groups to see if we hit anything
        # spawn weapons and logic in this group
        self.attack_sprites = pygame.sprite.Group()
        self.attackable_sprites = pygame.sprite.Group()  # spawn enemies in this group
    ...
    def create_magic(self, style, strength, cost):
        if style == 'heal':
            self.magic_player.heal(self.player, strength, cost, [
                                   self.visible_sprites])  # visible sprites since no collision with a heal spell

        if style == 'flame':  # we need visible and attack sprites for collision with flame spell
    ...
    # check if attack_sprite and attackable_sprite collide
                ...
                if collision_sprites:
                    for target_sprite in collision_sprites:
                        if target_sprite.sprite_type == 'grass':
                            ...
                            # spawn more leaf images for animation
                            for leaf in range(randint(3, 6)):
                                self.animation_player.create_grass_particles(
                                    pos - offset, [self.visible_sprites])
                            target_sprite.kill()  # instantly kills sprite
                        else:
